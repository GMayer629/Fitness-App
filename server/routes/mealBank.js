...
router.get('/', (req, res) => {
  res.json(db.prepare('SELECT * FROM meal_bank ORDER BY name').all());
});
...
router.post('/', (req, res) => {
  ...
  res.json({ id: result.lastInsertRowid, name, calories, protein });
});
...
router.post('/:id/add', (req, res) => {
  ...
  res.json({ id: result.lastInsertRowid, name: meal.name, calories: meal.calories, protein: meal.protein, date });
});
